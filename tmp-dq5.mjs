...
for (const HADITH_ID of [20925]) {
  const src = await pool.query(`
    SELECT DISTINCT ON (n.id, hjl.service_main_id)
           n.id as scientist_id, n.abb_name,
  ...
  const judgments = await pool.query(`
    ...
    WHERE j.hadith_id = $1
    ORDER BY n.death_year_num ASC NULLS LAST LIMIT 30
  `, [HADITH_ID])
  ...
  for (const j of judgments.rows) {
    ...
    console.log(`  [${j.abb_name}] → ${chosen ? `${chosen.book_name} (${chosen.part_num}/${chosen.page_num})` : 'no source'}`)
  }
}
