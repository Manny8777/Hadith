...
const checks = [
  ...
  { path: '/api/hadith/7/commentary', type: 'json', validate: body => JSON.stringify(body).includes('سيرة') },
  ...
  { path: '/hadiths/services-index', type: 'html', validate: body => { const clean = body.replace(/<!--.*?-->/g, ''); return clean.includes('hadith_services') && clean.includes('شبهات') && clean.includes('TypeID 15') } },
